...
* The code here is 
* I have not made this demo to use classes / objects for the sake of understanding.
* 
*
*/
...
int main(HINSTANCE hinst, HINSTANCE hpinst, LPSTR cmdline, int ncmdshow)
{
	...
	float r = 0;
	float scale = 1;
	float  x = 0, y = 0, z = 0; // temporary float varibles used for some quick math
	float fontSize = 0.08;
	...
	// These examples are sharing some varibles. 
	...
	voxie->menuReset(menu_example_update, 0, (char*)"test.png");
	...
	while (voxie->breath())
	{
		...
		if (voxie->getKeyIsDown(KB_Q)) { fillMode++; } // press 'Q' to increase fillmode
		if (voxie->getKeyIsDown(KB_A)) { fillMode--; } // press 'A' to decrease fillmode
		...
		*/
		...
		// altneratively you can pass the positional data as floats	
		pos		= { -.5, 0.35, -0.15 };
		pos2	= { -.2, 0.65, 0.15 };
