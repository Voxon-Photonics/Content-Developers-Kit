 ...
 *  calling this function createsa new 'VoxieBox' an object Once the VoxieBox object is created it can be accessed using an 
 ...
 * main header file for VX++ development. Requires vxDataTypes.h and vxInputTypes.h to work.
...
*
*   For detailed atritube and methods information see the member infomation on @see VoxieBox
*/
class IVoxieBox {
public:
...
// Deconstructor
	...
	// Deconstructor for VoxieBox automatically calls shutdown() and frees the .DLL from memory.
	virtual ~IVoxieBox() { ; }

// Core Functions 


	//! A 'breath' is the 'update loop' for a VX application. A single breath is a single volume. breath() passes in the defeult voxie_window and input
	...
	//! Signifies the start of the volumetric frame. Call this once before all draw calls. Prepares the internal vf voxie_frame_t() voxel buffer to recieve volumetric content.  
	...
	//! First time it is called it intalises the voxie_wind_t. Subsequent calls updates the VoxieBox.dll's voxie window struct
	virtual  int init() = 0;
...
// Settings
	...
	//! Toggles when enabled and VoxieBox::quitLoop() is called the Voxiebox libary is freed from memory. Set to true by default.
	...
	//! Set to false to disable the VoxieBox class / VxCpp.dll to from managing gamepad / joy input handling. Set to false bt default. 
...
// Time
	...
	//! Returns in seconds the delta time(time between volumes) delta time is CPU speed dependant and can be used to make ensure timing is consistent between various computers / systems
...
// Display
	...
	//! returns the interal voxie_wind_t's aspect ratio values data as a point3d 
	...
	//! set the interal voxie_wind_t's aspect ratio values data as a point3d 
	...
	//! Sets the 'view' for the volumetric display by default it is the aspect ratio of the voxie window but can be overiden
	...
	//! Frees a filename from VoxieBox.dll internal cache (any filename passed to voxie_drawmeshtex(), voxie_drawheimap(), voxie_drawspr())
	virtual void	freeGfx(char *fileName) = 0;
	...
	//! Exposes all the varibles from the internal vw voxie_wind_t onto the secondary (touch) screen. For debuging
	virtual void	reportVoxieWind(int posX, int posY) = 0;
	//! Exposes all the varibles from the internal vf voxie_frame_t onto the secondary (touch) screen. For debugging
...
// Debug 
	...
	virtual double	debugBar(int posX, int posY, double currentVal, double maxVal, double minVal, char* text, int type) = 0;
	//! Displays volumes per second (volumetric equivalent of 'FPS') and version info on the secondary (touch) screen. Must be called within the startFrame() & endFrame() functions.
	virtual void	showVPS(int posX = 15, int posY = 525) = 0;
...
//  Drawing Graphics to Volumetric Display
	...
	virtual void drawMesh(char* fileName, poltex_t* verticeList, int verticeNum, int* meshList, int meshNum, int flags, int col) = 0;
	...
	virtual void drawQuad(char *filename, point3d *pos, float width, float height, float hAng, float vAng, float twist, int col, float uValue, float vValue) = 0;
...
// Keyboard
	...
	//! Similar to getKeyStream() but beturns only the scancode of the keystate of a streaming state. Returns 0 if no more keys are in the stream. Requires setEnableLegacyKeyInput is set to true.
...
// Mouse
	...
	//! Set the Mouse's X and Y sensitivity. 0.001 is default. (0.0001 = low sensitive, 0.9 = extremely sensitive) Mouse movements work in intergers the amount is scaled down
	virtual void			setMouseXYSensitivity(float newAmount) = 0;
	//! Set the Mouse's Z sensitivity. 0.0005 is default. (0.0001 = low sensitive, 0.9 = extremely sensitive) Mouse movements work in intergers the amount is scaled down
	...
	//! Returns the state of mouse's buttonCode. 1 if currently pressed down and 0 if not being pressed. (buttonCodes are 0 for left, 1 for right, 2 for centre buttons) 
	...
	//! checks to see if desired MoustButton has been just released ('on up'). (buttonCodes are 0 for left, 1 for right, 2 for centre buttons) 
	virtual int				getMouseButtonOnUp(int buttonCode ) = 0;
	//! Returns the mount of time (in seconds) a mouse's button has been pressed. (buttonCodes are 0 for left, 1 for right, 2 for centre buttons)  
...
// Touch Controls
	...
	virtual void			touchAddKey(const touchkey_t *touchkey, int num) = 0;
...
// Game Controllers
	...
	virtual int			updateJoyState(int controllerID, voxie_xbox_t *vx) = 0;
	...
	//! Returns 1 if controller's button is pressed down. See vxInputTypes::JoyButtonCodes for refrence on Joy Button Codes  
	virtual int			getJoyButtonIsDown(int controllerID, int joyButtonCode) = 0;
	//! Returns 1 if controller's button is just released.  See vxInputTypes::JoyButtonCodes for refrence on Joy Button Codes.
	virtual int			getJoyButtonOnUp(int controllerID, int joyButtonCode) = 0;
	//! Returns 1 if controller's button is just pressed. See vxInputTypes::JoyButtonCodes for refrence on Joy Button Codes.
	...
	//! Sets the internal dead zone value for game controlers. (dead zone is the area where not input is detected). Value between 0 and 1. 0 = no deadzone. 1 = all deadzone. default is 0.3;
...
// Space Mouse
	...
	virtual int			updateNavState(int spaceNavID, voxie_nav_t *nav) = 0;
	...
	//! Returns a point3d of the Nav's tracked position. Used primarly when the Space Nav is being used as a cursor.
	virtual point3d		getNavPosition(int spaceNavID) = 0;
	//! Returns a single axis of a Space Nav's direction delta (range is -1 to 1, 0 is centred). Axis 0 = x, 1 = y, 2 = z
	virtual float		getNavDirectionDeltaAxis(int spaceNavID, int axis) = 0;
	//! Returns a single axis of a spaceNav's angle delta (range is -1 to 1, 0 is centred). Axis 0 = x, 1 = y, 2 = z
...
// Menus
	...
	virtual void menuAddTab(char *st, int x, int y, int xs, int ys) = 0;
	//! Add an item to a menu tab
	virtual void menuAddItem(char *st, int x, int y, int xSize, int ySize, int id, int type, int state, int col, double startingVal, double minVal, double maxVal1, double minStepVal, double majStepVal) = 0;
	//! Once the menu is created, use this function to update a string, button status, or slider value
	virtual void updateMenu(int id, char *st, int state, double v) = 0;
...
// Misc
	...
	virtual void	mountZip(char *fileName) = 0;
...
// Ultilies
	...
	//! Returns a scrolling color as an RGB hexidemical value 
	virtual int		scrollCol(int offSet) = 0;
	...
	//! Compare two point3d with a degree of accuracy. returns true if the two points are the same within the accuracy ammount specified
	virtual int		pointSame(point3d * a, point3d * b, point3d accuracy) = 0;
	//! Compare two point2d with a degree of accuracy. returns true if the two points are the same within the accuracy ammount specified
	virtual int		pointSame(point2d * a, point2d * b, point2d accuracy) = 0;
	//! Divides a RGB hexidemical color value by a division number (use this as when using drawModel() the color values are a 4th of the range)
...
*  The VoxieBox class is all the voxiebox.dll functions, helper functions and interal variables to manage a Voxon volumetric application.  
...
*  with OO prinpicals and C++ development easier for Voxon applications.
*/
...
public:
	...
	voxie_wind_t		vw;			//!< internal voxie_wint_t struct. This attribute will hold all the Voxon window data for this VoxieBox object. 
									//!< technically this data should be private, but it has been made public to allow direct access to for advanced users */
	voxie_frame_t		vf;			//!< internal voxie_frame_t struct. This attribute manages what is being drawn to the volumetric screen.  technically this data should be private, but it has been made public to allow direct access to for advanced users 2*/
	...
	double	debugBar(int posX, int posY, double currentVal, double maxVal, double minVal, char* text, int type);
	void	showVPS(int posX, int posY);
	...
	void drawMesh(char *fileNam, poltex_t *verticeList, int verticeNum, int *meshList, int meshNum, int flags, int col);
	...
	// Touch Controls
	void	touchAddKey(const touchkey_t *touchkey, int num);


	// Game Controllers
	int			updateJoyState(int controllerID, voxie_xbox_t *vx);
	...
	// Space Mouse
	int			updateNavState(int spaceNavID, voxie_nav_t *nav);
	int		    getNavNum();
	...
	void menuAddTab(char *st, int x, int y, int xs, int ys);
	void menuAddItem(char *st, int x, int y, int xSize, int ySize, int id, int type, int state, int col, double startingVal, double minVal, double maxVal1, double minStepVal, double majStepVal);
	void updateMenu(int id, char *st, int state, double v);
...
private:
	...
	point3d					mousePos = { 0,0,0 };					//!< the interal mouse position used for mouse clipping and displaying a mouse cursor. Use VoxieBox::getMousePosition() to return values
	...
	double durationLastPressedMouse[3] = { 0 };						//!< timers to deterkine the last pressed mouse duration. Used internally.
	...
	int		navOri[4] = { 0,0,0,0 };								//!< the orientation settings for the Spavce Nav controllers can be adjusted using VoxieBox::setNavOrientation() and VoxieBox::getNavOrientation()
	int		mouseOri = 0;									
	...
	double oldTime;													//!< Internal variable to manage previous volume's time. Used to calulate deltatime
	...
	bool manualJoyManage	= false;								//!< Internal bool to determine whether to use modern or legacy game controller input manmagement.
	bool manualNavManage	= false;								//!< Internal bool to determine whether to use modern or legacy Space Nav input manmagement.
	bool manualKeyManage	= false;								//!< Internal bool to determine whether to use modern or legacy keyboard input manmagement.
	...
	point2d		oriCorrection(int oriType, float xValue, float yValue);		//!< fixes an XY value to the predescribed orientation 
