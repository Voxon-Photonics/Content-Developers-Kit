...
//Ken's ZIP functions:
	...
	*/
	...
	//! Internal zip file management function. All 'kz...' functions are to do with Ken's zip file access
	virtual INT_PTR				_kzopen(const char *) = 0;
	//! Internal zip file management function. All 'kz...' functions are to do with Ken's zip file access
	virtual void				_kzfindfilestart(const char * st) = 0;   //pass wild-card string
	...
	virtual void				_kpzload(const char * fileName, INT_PTR * fptr, INT_PTR * bpl, INT_PTR * xsiz, INT_PTR * ysiz) = 0;
};
...
public:
	...
	*/
	...
	void				_kzsetfil(FILE * fileName);
	INT_PTR				_kzopen(const char * st);
	void				_kzfindfilestart(const char * st);   //pass wildcard string
	...
	void				_kpzload(const char * fileName, INT_PTR * fptr, INT_PTR * bpl, INT_PTR * xsiz, INT_PTR * ysiz);
...
#pragma region DLL_Functions
	...
	//High-level (easy) picture loading function:
	void(__cdecl *kpzload)(const char *fileName , INT_PTR * fptr, INT_PTR * bpl, INT_PTR *xsiz , INT_PTR * ysiz);
	//Low-level PNG/JPG functions:
	...
	void(__cdecl *kzuninit)();
	void(__cdecl *kzsetfil)(FILE * fileName);
	INT_PTR(__cdecl *kzopen)(const char *);
	void(__cdecl *kzfindfilestart)(const char * st);
