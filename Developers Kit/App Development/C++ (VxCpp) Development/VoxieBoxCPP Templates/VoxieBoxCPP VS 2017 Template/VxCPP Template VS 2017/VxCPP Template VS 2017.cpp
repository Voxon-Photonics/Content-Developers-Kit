...
int main(HINSTANCE hinst, HINSTANCE hpinst, LPSTR cmdline, int ncmdshow)
{
	...
	// variables to setup text positions to display 'Hello World' -- feel free to delete this just 
	...
	while (voxie->breath())
	{
		...
		voxie->drawText(&textPos, &textWidth, &textHeight, 0xffffff, "G'day World"); // draw text onto the volumetric display.
