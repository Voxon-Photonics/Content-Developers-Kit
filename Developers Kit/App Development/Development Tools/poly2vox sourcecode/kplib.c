...
#elif defined(_MSC_VER) && !defined(_WIN64) && !defined(NOASM)
...
static inline int Paeth686 (int a, int b, int c)
{
	__asm__ __volatile__ (
		...
		"cmpl (%%edx,%%eax,4), %%edi \n"
		"cmovgel %%eax, %%ecx \n"
		: "+c" (c) : "a" (a), "b" (b), [abstab10] "m" (abstab10[0]) : "edx","esi","edi","memory","cc"
		);
	return c;
}
...
static inline void rgbhlineasm (int c, int d, INT_PTR t, int b)
{
	__asm__ __volatile__ (
		...
		"jnz 2b \n"
		"3: \n"
		: "+c" (c), "+d" (d), "+D" (t) : "b" (b), [trnsrgb] "m" (trnsrgb), [olinbuf] "m" (olinbuf[0]) : "eax","memory","cc"
		);
}
...
static inline void pal8hlineasm (int c, int d, INT_PTR t, int b)
{
	__asm__ __volatile__ (
		...
		"jnz 0b \n"
		"1: \n"
		: "+c" (c), "+d" (d), "+D" (t) : "b" (b), [kplib_palcol] "m" (kplib_palcol[0]), [olinbuf] "m" (olinbuf[0]) : "eax","memory","cc"
		);
}
...
//#pragma optimize("g",off) //VC12_64 works with this hack
//#pragma function(memcpy) //VC12_64 works with this hack
	...
	int daglobxoffs, int daglobyoffs)
{
	...
	const unsigned char *kfileptr, *kfileend;

	if (!kpeginited) { kpeginited = 1; initkpeg(); }

	kfileptr = (const unsigned char *)kfilebuf;
	kfileend = &kfileptr[kfilength];

	if (*(unsigned short *)kfileptr != SSWAPIB(0xd8ff)) return(-1); //"%s is not a JPEG file\n",filename
	kfileptr += 2;
	...
	do
	{
		...
		switch(marker)
		{
			case 0xc0: case 0xc1: case 0xc2:
				...
				kfileptr += 5;
				ghsampmax = 0; gvsampmax = 0; glhsampmax = 0; glvsampmax = 0;
				for(z=0;z<gnumcomponents;z++)
				{
					...
					for(i=0;i<8;i++) if (gcomphsamp[z] == pow2int[i]) { gcomphsampshift[z] = i; break; }
					for(i=0;i<8;i++) if (gcompvsamp[z] == pow2int[i]) { gcompvsampshift[z] = i; break; }
			...
			case 0xc4:  //Huffman table
				do
				{
					...
					for(i=1;i<=16;i++)
					{
						...
						//memcpy((void *)&huftable[index][hufcnt[index]],(void *)kfileptr,(int)hufnumatbit[index][i]); /VC12_64:bad with this
						memmove((void *)&huftable[index][hufcnt[index]],(void *)kfileptr,(int)hufnumatbit[index][i]); //VC12_64:works with this
						if (i <= 10)
						{
							z = (int)hufnumatbit[index][i];
							for(c=0;c<z;c++)
								for(j=(1<<(10-i));j>0;j--)
								{
			...
			case 0xda:
				...
				{
					for(z=0,zz=0;z<gnumcomponents;z++)
					{
						dctx[z] = ((xdim+(ghsampmax<<3)-1)>>(glhsampmax+3)) << gcomphsampshift[z];
						dcty[z] = ((ydim+(gvsampmax<<3)-1)>>(glvsampmax+3)) << gcompvsampshift[z];
						zz += dctx[z]*dcty[z];
					}
					z = zz*64*sizeof(dctbuf[0]);
					dctbuf = (short *)malloc(z); if (!dctbuf) return(-1);
					memset(dctbuf,0,z);
					for(z=0,zz=0;z<gnumcomponents;z++) { dctptr[z] = &dctbuf[zz*64]; zz += dctx[z]*dcty[z]; }
				}

				glhstep = 0x7fffffff; glvstep = 0x7fffffff;
				for(z=0;z<lnumcomponents;z++)
							...
							for(yy=0;yy<(lcompvsamp[c]<<3);yy+=8)
								...
								{  //NOTE: Might help to split this code into firstime vs. refinement (!Ah vs. Ah!=0)
									...
									if (!Ss)
									{
										...
										else if (num&(pow2int[--curbits])) dcs[0] |= ((short)Alut[0]);
									}
										...
										for(;z<=Se;z++)
										{
											...
											if (daval)
											{
												if (Ah)
												{
													...
													if (num&(pow2int[--curbits])) daval = Alut[0]; else daval = Alut[1];
												}
											}
											else if (zz < 15)
											{
											...
											if (Ah)
											{
												do
												{
													...
													{
														KPEG_GETBITS(curbits,8,num,kfileptr,kfileend);
														if (num&(pow2int[--curbits])) dcs[z] += (short)Alut[dcs[z] < 0];
												  } else if (--zz < 0) break;
												  z++;
												} while (z <= Se);
...
//#pragma optimize("g",on) //VC12_64 works with this hack
//#pragma intrinsic(memcpy) //VC12_64 works with this hack
...
FUNCPREFIX int kpgetdim (const char *buf, int leng, int *xsiz, int *ysiz)
{
	int *lptr;
	...
	else if ((ubuf[0] == 'B') && (ubuf[1] == 'M')) //.BMP
	{
		...
		else //All newer formats...
		{
			...
			(*xsiz) = labs(LSWAPIB(*(int *)&buf[18]));
			(*ysiz) = labs(LSWAPIB(*(int *)&buf[22]));
		}
		return(KPLIB_BMP);
	}
...
FUNCPREFIX int kprender (const char *buf, int leng, INT_PTR frameptr, int bpl,
...
//===================== ZIP decompression code begins ========================
	...
	//[char zipnam[?]\0]
	//[next hashindex/-1][next index/-1][zipnam index][fileoffs][fileleng(uncomp)][iscomp/compleng][char filnam[?]\0]
	//[next hashindex/-1][next index/-1][zipnam index][fileoffs][fileleng(uncomp)][iscomp/compleng][char filnam[?]\0]
	//...
	//[char zipnam[?]\0]
	//[next hashindex/-1][next index/-1][zipnam index][fileoffs][fileleng(uncomp)][iscomp/compleng][char filnam[?]\0]
...
static int kzhashead[256], kzhashpos, kzlastfnam = -1, kzhashsiz, kzdirnamhead = -1;
...
static int kzcheckhash (const char *filnam, char **zipnam, int *fileoffs, int *fileleng, int *iscomp)
{
	int i;
...
FUNCPREFIX int kzaddstack (const char *filnam)
{
	FILE *fil;
	...
	if (i == LSWAPIB(0x04034b50)) //'PK\3\4' is ZIP file id
	{
		...
		for(i=0;i<numfiles;i++)
		{
			...
			j = strlen(&tempbuf[46])+25; if (!kzcheckhashsiz(j)) { fclose(fil); return(-1); }
			...
			*(int *)&kzhashbuf[kzhashpos+16] = LSWAPIB(*(int *)&tempbuf[24]); //fileleng (uncomp) used only when local file header is 0 (rare)
			*(int *)&kzhashbuf[kzhashpos+20] = LSWAPIB(*(int *)&tempbuf[20]); //fileleng (comp) used only when local file header is 0 (rare); formerly iscomp (non zero = compressed)
			strcpy(&kzhashbuf[kzhashpos+24],&tempbuf[46]);
	...
	else if (i == LSWAPIB(0x536e654b)) //'KenS' is GRP file id
	{
		...
		for(i=0;i<numfiles;i++,k+=leng)
		{
			...
			tempbuf[12] = 0;
				...
				//Write information into hash
			j = strlen(tempbuf)+25; if (!kzcheckhashsiz(j)) { fclose(fil); return(-1); }
			...
			*(int *)&kzhashbuf[kzhashpos+20] = 0; //iscomp
			strcpy(&kzhashbuf[kzhashpos+24],tempbuf);
			kzhashead[hashind] = kzhashpos; kzlastfnam = kzhashpos; kzhashpos += j;
		}
	}
...
FUNCPREFIX void kzsetfil (FILE *fil)
{
	kzfs.fil = fil;
	kzfs.comptyp = 0;
	kzfs.seek0 = 0;
...
FUNCPREFIX INT_PTR kzopen (const char *filnam)
{
	FILE *fil;
	int i, j, fileoffs, fileleng, iscomp;
	...
	if (kzcheckhash(filnam,&zipnam,&fileoffs,&fileleng,&iscomp)) //Then check mounted ZIP/GRP files
	{
		...
		else
		{
			...
			if (tempbuf[6]&8) { kzfs.leng = fileleng; } //Fixed 11/27/2017: GPFlags says local header invalid; must get leng from central header
			...
			switch(kzfs.comptyp) //Compression method
			{
				...
				case 8:
					...
					if (tempbuf[6]&8) { kzfs.compleng = iscomp; } //Fixed 11/27/2017: GPFlags says local header invalid; must get leng from central header
...
FUNCPREFIX void kzfindfilestart (const char *st)
{
#if defined(__DOS__)
#elif defined(_WIN32)
	...
	srchstat = 0; if ((st[0] == ':') && (!st[1])) { srchstat = 10; } //Calling kzfindfilestart(":"); results in kzaddstack() string list
	srchzoff = kzlastfnam; srchdoff = kzdirnamhead;
}

FUNCPREFIX int kzfindfile (char *filnam)
{
	int i;
...
kzfindfile_beg:;
	...
	while (srchstat == 2)
	{
		...
		if (wildmatch(&kzhashbuf[srchzoff+24],newildst))
		{
			//strcpy(filnam,&kzhashbuf[srchzoff+24]);
			filnam[0] = '|'; strcpy(&filnam[1],&kzhashbuf[srchzoff+24]);
	...
	if (srchstat == 10)
	{
		if (srchdoff < 0) { srchstat = 11; }
		else
		{
			strcpy(filnam,&kzhashbuf[srchdoff+4]);
			srchdoff = *(int *)&kzhashbuf[srchdoff];
			return(1);
		}
	}
	if (srchstat == 11) //zip/grp names..
	{
		if (srchzoff < 0) { srchstat = -1; return(0); }
		i = *(int *)&kzhashbuf[srchzoff+8];
		strcpy(filnam,&kzhashbuf[i]);
		while (srchzoff >= 0) //each file points to its zip name.. here we search all files until new zip name found.
		{
			srchzoff = *(int *)&kzhashbuf[srchzoff+4];
			if (*(int *)&kzhashbuf[srchzoff+8] != i) break;
		}
		return(1);
	}
...
FUNCPREFIX void kzuninit (void)
{
	if (kzhashbuf) { free(kzhashbuf); kzhashbuf = 0; }
	kzhashpos = 0; kzlastfnam = -1; kzhashsiz = 0; kzdirnamhead = -1;
	srchzoff = 0; srchdoff = -1;
}
...
FUNCPREFIX int kzread (void *buffer, int leng)
{
...
FUNCPREFIX int kzfilelength (void)
{
...
FUNCPREFIX int kzseek (int offset, int whence)
{
...
FUNCPREFIX int kztell (void)
{
	if (!kzfs.fil) return(-1);
	return(kzfs.pos);
}

FUNCPREFIX int kzgetc (void)
{
	char ch;
...
FUNCPREFIX int kzeof (void)
{
	if (!kzfs.fil) return(-1);
	return(kzfs.pos >= kzfs.leng);
}

FUNCPREFIX void kzclose (void)
{
...
FUNCPREFIX void kpzload (const char *filnam, INT_PTR *pic, int *bpl, int *xsiz, int *ysiz)
{
	char *buf;
	int leng;
