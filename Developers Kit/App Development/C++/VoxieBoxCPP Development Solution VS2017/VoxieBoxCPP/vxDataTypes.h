...
typedef struct uvcol_t {
	
	float u,		//!< the U value for a texture's UV value
		  v;		//!< the V value for a texture's UV value
	  int col;		//!< 24-bit hexadecimal colour value for color of vertex.  

} uvcol_t;
//! struct to manage indices for VoxieBox::drawMeshExt() function 
//! XYZ index and UV + C (color) index 
//! ! Important ! when defining a new polygon loop / face use the .xyzi + (1<<31); to indicate a new face
typedef struct inds_t {
	int xyzi,   // xyz index
		uvci;   // uvc (UV and Color) index
} inds_t; //if (xyzi&0x80000000) start new loop (1<<31)

static const int EXT_START_LOOP = (1<<31); 
