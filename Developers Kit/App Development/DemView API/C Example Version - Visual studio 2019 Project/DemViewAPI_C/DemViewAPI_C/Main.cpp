/* Demview Simp API working in Visual Studio 
	Executable requires DemView.DLL
 */
...
int main(HINSTANCE hinst, HINSTANCE hpinst, LPSTR cmdline, int ncmdshow)
{
	...
	while (!voxie_breath(&in))
	{
		...
		if (voxie_keystat(0x2) == 1) { if (demview_setserv(1, 0, 20, 8, 1, 1, 1, 8, 1, (char*)"cache/vi2%d_%dx%dx%d.jpg", (char*)"t0.ssl.ak.tiles.virtualearth.net", (char*)"/tiles/a%s.jpeg?g=5290", 1.f, 1.f, 1.f)) return(1); }
		if (voxie_keystat(0x3) == 1) { if (demview_setserv(1, 0, 20, 8, 1, 0, 1, 8, 5, (char*)"cache/vir%d_%dx%dx%d.png", (char*)"ecn.t0.tiles.virtualearth.net", (char*)"/tiles/r%s.png?g=5273", 1.f, 1.f, 1.f)) return(1); }
