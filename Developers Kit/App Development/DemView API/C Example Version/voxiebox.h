...
enum {VOLCAP_OFF=0, VOLCAP_FRAME_PLY, VOLCAP_VIDEO_PLY, VOLCAP_FRAME_PNG, VOLCAP_FRAME_REC, VOLCAP_VIDEO_REC, VOLCAP_FRAME_VCB, VOLCAP_VIDEO_VCBZIP, VOLCAP_NET};
...
int voxie_load (voxie_wind_t *vw)
{
	...
	{
		int i = GetLastError();
		if (i == 0xc1) { MessageBox(0,"FYI: Are you mixing a 32-bit EXE with a 64-bit DLL?","ERROR_BAD_EXE_FORMAT (0xc1) not valid Win32 app.",MB_OK); }
		return(-1);
	}
